...
CATEGORY_MAPPING = {
    ...
    # Служебные — не жалобы, оставлены отдельной колонкой для прозрачности,
    # а не растворены в общем "Другое".
    'Копия тикета': 'Копия тикета',
    'СПАМ': 'СПАМ',
}
...
# Служебные значения "Причина обращения" — это не жалоба гостя, а пометка
# оператора об административном статусе тикета. Категоризировать их текстом
# нельзя (внутри может быть жалоба из ДРУГОГО, оригинального тикета) — они
# просто исключаются из содержательных категорий, но остаются видны отдельно.
NON_COMPLAINT_REASONS = {
    'копия тикета': 'Копия тикета',
    'спам': 'СПАМ',
}

# "Причина обращения" в CRM для многих тикетов — уже готовая, однозначная
# категория (например "Опоздание менее 30 минут"). Такой явной причине
# доверяем напрямую, не прогоняя её через нечёткий keyword-поиск по тексту.
# Ключи — причины с частотой >=2 в реальной выгрузке, для которых категория
# читается однозначно; всё остальное (в т.ч. общие "другое"/"Запрос") — как
# раньше, по ключевым словам в тексте обращения.
REASON_CATEGORY_MAP = {
    'опоздание менее 30 минут': 'Опоздания',
    'опоздание 30 - 60 минут': 'Опоздания',
    'опоздание 60 - 90 минут': 'Опоздания',
    'опоздание 90 - 120 минут': 'Опоздания',
    '20-45 минут с момента заказа': 'Опоздания',
    'нарушение сроков доставки': 'Опоздания',
    'нарушение сроков самовывоз / зал': 'Опоздания',
    'как получить скидку на день рождения': 'Скидка на День Рождения',
    'не пришёл промо-код на день рождения': 'Скидка на День Рождения',
    'бонусная программа': 'Проблема с бонусами или промокодом',
    'как использовать папа бонусы/баллы': 'Проблема с бонусами или промокодом',
    'промокод не срабатывал': 'Проблема с бонусами или промокодом',
    'промо-код не попал в заказ': 'Проблема с бонусами или промокодом',
    'проблема с баллами': 'Проблема с бонусами или промокодом',
    'проблема с промо-кодом': 'Проблема с бонусами или промокодом',
    'баллы': 'Проблема с бонусами или промокодом',
    'баллы сгорели': 'Проблема с бонусами или промокодом',
    'не вернулись баллы после отмены заказа': 'Проблема с бонусами или промокодом',
    'сложность в оформлении заказа': 'Приложение / Сайт / IT-сбои',
    'заказ не попал в систему': 'Приложение / Сайт / IT-сбои',
    'не отображается информация': 'Приложение / Сайт / IT-сбои',
    'выдаёт внутреннюю ошибку приложения': 'Приложение / Сайт / IT-сбои',
    'не доставили позицию из чека': 'Забыли позицию',
    'комплектация и сервировка заказа': 'Забыли позицию',
    'отсутствие порционных перчика/соуса (к трад тесту)': 'Забыли позицию',
    'отсутствие соуса по рецептуре': 'Забыли позицию',
    'отсутствие дополнительных ингредиентов в пицце': 'Забыли позицию',
    'отсутствие ингредиентов по рецептуре': 'Забыли позицию',
    'возврат': 'Возврат ДС',
    'не оформили возврат средств в день запроса': 'Возврат ДС',
    'заказ закрыт раньше, чем была произведена доставка': 'Не доставили заказ',
    'не доставили': 'Не доставили заказ',
    'вкусовые качества и состав': 'Невкусно',
    'не вкусно': 'Невкусно',
    'сухая, жесткая пицца': 'Невкусно',
    'сухая': 'Невкусно',
    'перепутали позицию из чека': 'Перепутали позицию',
    'перепутали борт': 'Перепутали позицию',
    'перепутали тесто': 'Перепутали позицию',
    'перепутали заказ': 'Перепутали позицию',
    'не убрали ингредиенты': 'Перепутали позицию',
    'внешний вид блюда': 'Некачественно',
    'качество доставленного заказа': 'Некачественно',
    'подгоревшая': 'Некачественно',
    'мало ингредиентов': 'Некачественно',
    'неравномерно распределенная начинка': 'Некачественно',
    'сырое тесто': 'Некачественно',
    'смена данных': 'Управление аккаунтом',
    'удаление аккаунта': 'Управление аккаунтом',
    'объединение аккаунтов': 'Управление аккаунтом',
    'восстановление пароля': 'Управление аккаунтом',
    'клиент просит удалить аккаунт': 'Управление аккаунтом',
    'отмена процедуры удаления аккаунта': 'Управление аккаунтом',
    'не дозвониться': 'Не дозвониться',
    'мятая пицца (смятый край)': 'Разлит / Повреждение упаковки',
    'мятая пицца': 'Разлит / Повреждение упаковки',
    'мятая': 'Разлит / Повреждение упаковки',
    'помятая': 'Разлит / Повреждение упаковки',
    'перевёрнутая': 'Разлит / Повреждение упаковки',
    'упаковка': 'Разлит / Повреждение упаковки',
    'инородный предмет': 'Инородный предмет в блюде',
    'отравление': 'Отравление',
    'холодная пицца': 'Холодная пицца',
    'холодная': 'Холодная пицца',
    'оскорбления.': 'Жалоба на сервис',
    'жалоба на курьера': 'Жалоба на сервис',
    'жалоба на доставку': 'Жалоба на сервис',
    'курьер забыл терминал, просил оплатить переводом': 'Жалоба на сервис',
    'не выдали сдачу': 'Жалоба на сервис',
    'курьер не прочитал комментарий к заказу по доставке. (позвонить, не звонить в дверь и т.д.)': 'Жалоба на сервис',
    'недовольство стоимостью за доставку во время высокого спроса.': 'Слишком дорого',
}
...
def categorize_complaint_detailed(text, reason):
    ...
    Порядок приоритета:
    1) служебная причина (копия/спам) — не жалоба, отдельная метка;
    2) причина обращения из CRM однозначно читается — доверяем ей напрямую;
    3) иначе — как раньше, по ключевым словам в тексте + причине.
    """
    reason_norm = str(reason or '').strip().lower()
    if reason_norm in NON_COMPLAINT_REASONS:
        return NON_COMPLAINT_REASONS[reason_norm]
    if reason_norm in REASON_CATEGORY_MAP:
        return REASON_CATEGORY_MAP[reason_norm]
    ...
    rules = [
        ...
        # ВАЖНО: 'др' как отдельное ключевое слово раньше ловил "зДРавствуйте" —
        # приветствие бота есть почти в каждом диалоге, из-за чего категория
        # разбухала до трети всех обращений. Оставлены только словосочетания.
        (['день рождения', 'день рождени', 'скидка на др', 'промокод на день', 'подарок на др', 'на днях был др', 'дню рождении', 'днем рождения', 'не пришёл промокод'], 'Скидка на День Рождения'),
