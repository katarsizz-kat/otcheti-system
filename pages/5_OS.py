"""Страница отчёта ОС (Отдел Обратной Связи)."""
...
/* ===== ГРАФИКИ ===== */
.stPlotlyChart, .stBokehChart, .stAltairChart {
    background: rgba(255, 255, 255, 0.95) !important;
    border-radius: 12px !important;
...
def main():
    ...
    <div class="header-block">
        <h1>💬 Отчёт по работе Отдела Обратной Связи (ОС)</h1>
        <p>{greeting_by_time()}</p>
        <p style="margin-top:10px; margin-bottom:0; font-size:16px;">
            Анализ обращений, жалоб и работы операторов
        </p>
    </div>
    """, unsafe_allow_html=True)
