...
- убраны критический #D6EAF8 и дублирующий set_page_config;
- локальные стили переведены на CSS-переменные (убран teal #005F6B,
  белый оверлей .stApp::before и text-shadow);
- карточки завершённых событий — на переменных;
- render_theme_controls() — блок "Оформление" в сайдбаре.
Вся бизнес-логика (Excel/PDF/Telegram, формулы) не тронута.
"""
...
# =============================================================================
# 2. ТЕМА (lite-режим) + БЛОК "ОФОРМЛЕНИЕ"
# =============================================================================
...
# =============================================================================
# 3. ЛОКАЛЬНЫЕ СТИЛИ СТРАНИЦЫ (на CSS-переменных)
# =============================================================================
st.markdown("""
<style>
/* ===== БЛОК КАЛЕНДАРЯ ===== */
...
# =============================================================================
# ЗАГОЛОВОК СТРАНИЦЫ (общий темизированный .header-block)
# =============================================================================
st.markdown("""
...
with tab_list:
    ...
    if st.session_state.show_edit_form and st.session_state.selected_event:
        ...
        with rc1:
            reminder_days_before_start = st.number_input("За сколько дней до НАЧАЛА", min_value=0, max_value=90, value=st.session_state.data_reminder_days_start, key="edit_input_reminder_days_start")
        with rc2:
            reminder_days_before_end = st.number_input("За сколько дней до ОКОНЧАНИЯ", min_value=0, max_value=90, value=st.session_state.data_reminder_days_end, key="edit_input_reminder_days_end")
        with rc3:
        ...
        with sc1:
            if st.button("💾 Сохранить", use_container_width=True, type="primary", key="btn_save_edit"):
                if not title:
                    ...
                    st.error("❌ Дата начала позже окончания")
                else:
        ...
        for event in completed_events:
            ...
            with col_info:
                ...
                ">
                    ...
                    <div style="color: var(--text-secondary); opacity: 0.7; font-size: 12px; margin-top: 5px;">
...
with tab_add:
    ...
    rc1, rc2, rc3 = st.columns(3)
    with rc1:
        reminder_days_before_start = st.number_input("До НАЧАЛА (дней)", min_value=0, max_value=90, value=st.session_state.data_reminder_days_start, key="input_reminder_days_start")
    with rc2:
        reminder_days_before_end = st.number_input("До ОКОНЧАНИЯ (дней)", min_value=0, max_value=90, value=st.session_state.data_reminder_days_end, key="input_reminder_days_end")
    with rc3:
    ...
    with sc1:
        if st.button("💾 Сохранить", use_container_width=True, type="primary", key="btn_save_event"):
            if not title:
                ...
                st.error(f"❌ Дата начала ({start_date}) позже окончания ({end_date})")
            else:
                location_code = cfg.LOCATIONS.get(location_type, "all")
                if not use_custom_date:
                    reminder_custom_date = None
